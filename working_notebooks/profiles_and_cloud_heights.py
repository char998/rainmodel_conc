...
from physics_equations import w_sat,specific_humidity_from_rh, w_sat_symbolic
...
def T_v(T_d,p_surf,p_air,T_arr,lcl_index):
    ...
    T_v_moist = T_arr*(1 +0.61*q_v - q_l)   #virtual temperature profile
    return T_v_moist,q_v



def insert_lcl_values(temp_parcel,p,Z,lcl_index,T_s_value,p_s_value,my_temp,RH_env,q_v):
    ...
    #when we don't have the lcl height exactly
        ...
        Z_lcl = R*np.mean(temp_parcel[0:lcl_index+2])*np.log(p[0]/p[lcl_index+1])/g #interpolation to Find the exact LCL height
        Z = np.insert(Z,lcl_index+1,Z_lcl)                          #insert the LCL height
        lcl_index = np.argwhere(Z == Z_lcl).item()                  #update the lcl index
        #-----------------------------------------------------------------------------------------------------------------------------------------------
        #air profiles update
        #find the corresponding air temperature and relative humidity (interpolations)
        ...
        Z_lcl = R*np.mean(temp_parcel[0:lcl_index+1])*np.log(p[0]/p[lcl_index])/g   #interpolation to Find the exact LCL height
...
def cloud_top(LFC_index,Z,Z_b,p,p_t,T_v_parcel,T_v_env,T_high,temp_parcel):
    ...
    #p_t: top pressure - if there is no LCL [Pa]
    #T_v_parcel: parcel virtual temperature profile [K]
    #T_v_env: environment virtual temperature profile  [K]
    #T_high: parcel temperature profile [K]
    #temp_parcel: parcel temperature profile [K]

    condition = T_v_parcel[LFC_index[1]:]<T_v_env[LFC_index[1]:]
    if LFC_index[0] == 'yes':
        if np.any(condition):
