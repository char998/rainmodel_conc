...
def ground_precipitation_rate(R, Dc,vb, a):
    """
    ...
    if R<=2.7e-5:      #excluding precipitation rates below 0.1 mm/h due to numerical issues
        P = 0

    else:
        v = vb           # [cm/s]
        Lambda = solve_lambda_mu_dependent(R, a, vb)
        mu = mu_from_lambda(Lambda)
        N0 = N0_from_mu(mu)
