...
def entrainment_theta(entrainment_rate,theta_conserved,Z,theta_air):
...
def entrainment_q(entrainment_rate,q_t,Z,q_air):
    ...
    for i in range(1, len(q_t_new)):
        q_t_new[i] = max(0,q_t_new[i-1] - entrainment_rate * (q_t_new[i-1] - q_air[i-1])*dZ[i-1])
