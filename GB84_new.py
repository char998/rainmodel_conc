#Altered GB84 model, so that cloud top and bottom velocities are independently defined/calculated
...
# In[63]:
def ambient_temp(T_0,p_0,p,T):
    #ambient temperature
    return min(T,T_0/(p_0**0.286)*(p)**0.286)
...
def v_conv(v0,T,T_ambient,Zc,dZ):
    #convective parametrization of velocity
    return v0 + (alpha_vel*g*(T/T_ambient - 1) - b_vel*v0/Zc)*dZ
...
#--------------------------------------------------------------------------------------------------------------------------
    def calculate_cloud_top(self,T_0,p_0,p_s,Theta_e):
        if self.obs:
            ...
            T_s_up = min(T_m,T_0 / (p_0/(p_s_up))**0.286) #T_s'
...
class h_out:
    def __init__(self,v_m,v,v2,Z_c,version):
        self.V_p = non_dim_numbers(v_m)[0]
        self.N_v = non_dim_numbers(v)[1]
        self.N_v2 = non_dim_numbers(v2)[1]
        self.Z_c = Z_c
    ...
    def O_t(self):
        if self.version == 1:
            ...
            return(1 + 3/4*(gamma*self.N_v2) + (gamma*self.N_v2)**2/4 + (gamma*self.N_v2)**3/24)\
...
class phi:
     ...
     def __init__(self, T_0, p_0, T_d, Z_b, Z_c,v_m,v,v2,version):
        self.T_0 = T_0
        self.p_0 = p_0
        self.T_d = T_d
        self.Z_b = Z_b
        ...
        self.v2 = v2
        self.version = version
        self.O_b = h_out(v_m,v,v2, Z_c,2).O_b
...
class state:
    def __init__(self,X,T_d,T_0,p_0,Z_c,Z_b,dt,p_t,T_t,rho,v_m,v,v2,version):
        self.X = X
        self.dt = dt
        self.T_d = T_d
        self.T_0 = T_0
    ...
    def state_evol(self):
        X_new = self.X + self.dt*(f(self.T_d,self.p_0,self.p_t,self.T_t,self.rho,self.v) \
                                  - h_out(self.v_m,self.v,self.v2,self.Z_c,self.version).run()[2]*self.X)
        if X_new<0:
         X_new = 0
        return X_new
